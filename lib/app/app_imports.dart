export 'package:flutter/material.dart';
export 'package:flutter/foundation.dart';
export 'package:get_it/get_it.dart';
export 'package:internet_connection_checker/internet_connection_checker.dart';
export 'package:dio/dio.dart';
export 'package:pretty_dio_logger/pretty_dio_logger.dart';
export 'package:json_annotation/json_annotation.dart';
export 'package:lottie/lottie.dart';
export 'dart:async';
export 'package:flutter/scheduler.dart';
export 'package:shared_preferences/shared_preferences.dart';


/// app
export 'package:tut_app/app/app.dart';
export 'package:tut_app/app/di.dart';
export 'package:tut_app/app/constants.dart';
export 'package:tut_app/app/extension.dart';
export 'package:tut_app/app/app_prefs.dart';


/// data
export 'package:tut_app/data/network/app_api.dart';
export 'package:tut_app/data/network/dio_factory.dart';
export 'package:tut_app/data/network/network_info.dart';
export 'package:tut_app/data/network/requests.dart';
export 'package:tut_app/data/network/error_handler.dart';
export 'package:tut_app/data/network/failure.dart';
export 'package:tut_app/data/data_source/remote_data_source.dart';
export 'package:tut_app/data/repository/repository_impl.dart';
export 'package:tut_app/data/mapper/mapper.dart';
export 'package:tut_app/data/response/response.dart';


/// domain
export 'package:tut_app/domain/repository/repository.dart';
export 'package:tut_app/domain/usecase/login_usecase.dart';
export 'package:tut_app/domain/model/models.dart';
export 'package:tut_app/domain/usecase/base_usecase.dart';
export 'package:tut_app/domain/usecase/register_usecase.dart';


/// presentation
export 'package:tut_app/presentation/resources/routes_manager.dart';
export 'package:tut_app/presentation/resources/theme_manager.dart';
export 'package:tut_app/presentation/resources/assets_manager.dart';
export 'package:tut_app/presentation/resources/values_manager.dart';
export 'package:tut_app/presentation/resources/string_manager.dart';
export 'package:tut_app/presentation/auth/login/view_model/login_view_model.dart';
export 'package:freezed_annotation/freezed_annotation.dart';
export 'package:tut_app/presentation/base/base_view_model.dart';
export 'package:tut_app/presentation/common/freezed_data_classes.dart';
export 'package:tut_app/presentation/onBoarding/view_model/onBoarding_view_model.dart';
export 'package:tut_app/presentation/resources/color_manger.dart';
export 'package:tut_app/presentation/resources/constants_manager.dart';
export 'package:tut_app/presentation/resources/style_manager.dart';
export 'package:tut_app/presentation/onBoarding/view/onBoarding_view.dart';
export 'package:tut_app/presentation/auth/forget_password/forget_password_view.dart';
export 'package:tut_app/presentation/auth/login/view/login_view.dart';
export 'package:tut_app/presentation/auth/register/view/register_view.dart';
export 'package:tut_app/presentation/main/main_view.dart';
export 'package:tut_app/presentation/splash/splash_view.dart';
export 'package:tut_app/presentation/resources/font_manager.dart';
export 'package:tut_app/presentation/common/state_render/state_render_impl.dart';
export 'package:tut_app/presentation/common/state_render/state_render.dart';
export 'package:tut_app/presentation/auth/register/view_model/register_view_model.dart';

